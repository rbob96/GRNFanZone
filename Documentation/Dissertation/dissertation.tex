...
Section \ref{sec:testing} will reflect on the difficulties faced when
 writing tests for the project in Karma Jasmine, a test-runner and framework
 combination reccomended for use with our frontend technology. It will continue
...
Section \ref{sec:planning} will show how the project was planned out,
...
Section \ref{sec:agile} explains some of the agile practices we used in our 
 project.

Section \ref{sec:changemgmt} describes how the team used their VCS and managed the 
 project in day-to-day life. This will describe the lifecycle of an issue ticket in
 depth.
...
A club is the highest level entity in the hierarchy. A club can be the parent of many different
 ...
 player can play for more than one team - for example, Zander Fagerson plays for both the Scotland
 team at an international level, and for the Glasgow Warriors at a professional level.
...
Sharing news as posts is important to improve fan engagement as it allows
 ...
 that, as each post is slowly hidden under newer posts displaying fresher 
 information.

The profile pages for each profile-owner are designed to allow fairly static 
...
The GRN FanZone is currently deployed to \url{http://grnfanzone.firebaseapp.com}.
 ...
 are presented with a newsfeed of posts. This feed uses infinite scrolling to 
...
In terms of internationalisation support, we managed translations for: Romanian,
...
At the very start of the project, we used a shared Google Docs folder
 ...
 with ease and built-in continous integration.
...
GitLab also allowed us to prioritise our tickets using a number of metrics - 
 from the weight assigned to them to the labels. We gave \texttt{Botfix} tickets
 ...
 issues raised at client meetings but not specified in the requirements dicument). 

For our branching model we selected one called Gitflow\cite{gitflow}. At
 its core, there are two main branches - dev and master. The \texttt{master} branch
...
In the Gitflow model, dev and master are not comitted to. Therefore
...
We encouraged every team member to commit often, as it encouraged transparency
 in the team by making work completed more visible. This policy also meant 
 ...
 bigger, more confusing issues. After the development of the feature was done, the assignee
...
Another blocker for merge request was merge conflicts. They occured if Git could not
 ...
 greatly. The asignee was responsible for this, as they would know the most about their
...
On merge, the issue ticket was closed, the new code merged into \texttt{dev}
 and the featue branch being deleted. This meant that the repository was not 
